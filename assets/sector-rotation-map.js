(() => {
  ...
  const drawBase = (svg) => {
    ...
    addText(svg, labels.improving, 88, 78, "srm-quadrant srm-quadrant--improving");
    addText(svg, labels.leading, 932, 78, "srm-quadrant srm-quadrant--leading", "end");
    addText(svg, labels.lagging, 88, 570, "srm-quadrant srm-quadrant--lagging");
    addText(svg, labels.weakening, 932, 570, "srm-quadrant srm-quadrant--weakening", "end");
  ...
  const drawPoints = (root, svg, points) => {
    ...
    ordered.forEach((point, index) => {
      const quadrant = point.quadrant || quadrantFor(point);
      const color = colors[quadrant] || "#ffffff";
      ...
      const aria = `${point.label_zh_hant}，${labels[quadrant]}，相對趨勢 ${number.format(point.x)}，相對動能 ${number.format(point.y)}`;
      ...
      const halo = svgNode("circle", { cx: x, cy: y, r: 13, fill: color, class: "srm-point-halo" });
      const dot = svgNode("circle", { cx: x, cy: y, r: 5.5, fill: color, class: "srm-point-dot" });
      ...
      if (tooltip) {
        const showTooltip = () => {
          ...
          identity.textContent = `${point.symbol} · ${labels[quadrant]}`;
  ...
  const initialize = (root) => {
    try {
      ...
      const render = () => {
        ...
        const points = universe.horizons[horizon].points;
        ...
        chart.setAttribute("aria-label", `${universe.title_zh_hant} ${horizon}日產業輪動象限圖`);
      ...
      universeSelect.addEventListener("change", render);
